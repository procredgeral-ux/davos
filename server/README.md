...
```sh
~$ sudo linux-script.sh install /your/preferred/directory
~$ sudo -u server -H /bin/sh -c "cd; /bin/node src/index.js"
```
Using ```sudo -u server -H /bin/sh -c "cd; /bin/node src/index.js" ``` to launch the server increases security by running the process as an unprivileged, dedicated user with a limited shell and it is recommended to do so.

Currently, the server listens on the following addresses and ports:
...
## Custom Game modes
The server has support for custom game modes. To switch between game modes, change the value of "serverGamemode" in the configurations file to the selected game mode id and restart the server. The current supported game modes are:
